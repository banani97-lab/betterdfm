...
export interface BoardTrace { layer: string; widthMM: number; startX: number; startY: number; endX: number; endY: number; netName: string; netSource?: string }
export interface BoardPad { layer: string; x: number; y: number; widthMM: number; heightMM: number; shape: 'RECT' | 'CIRCLE' | 'OVAL' | 'DONUT' | 'POLYGON'; holeMM?: number; contour?: Array<{ x: number; y: number }>; netName: string; netSource?: string; refDes: string; packageClass?: string }
...
export interface BoardPolygon { layer: string; points: Array<{ x: number; y: number }>; holes?: Array<Array<{ x: number; y: number }>>; netName: string; netSource?: string }
